...
composite_one() {
  ...
  # Font size = 5% of image height; bandheight = 18%
  local font_size=$((h * 5 / 100))
  local band_height=$((h * 18 / 100))
  local watermark_size=$((w * 4 / 100))

  $IM "$raw" \
    \( -size "${w}x${band_height}" gradient:"#042C53AA"-"#042C5300" \) \
      -gravity north -composite \
    -gravity north -fill white -font "Helvetica-Bold" -pointsize "$font_size" \
      -annotate +0+$((h * 4 / 100)) "$headline" \
    -gravity southeast -fill white -font "Helvetica-Bold" -pointsize "$watermark_size" \
