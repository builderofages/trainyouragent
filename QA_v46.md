...
2. **Anthropic API key validity.** The 502 from chat could still be an invalid `ANTHROPIC_API_KEY` rather than just a model id issue — only Vercel env access can confirm. The new error surface (`upstream-error 401 authentication_error` vs `upstream-error 404 not_found_error`) will make root-cause obvious in the next prod test. No further action possible from this side.
