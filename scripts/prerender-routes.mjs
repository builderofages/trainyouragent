...
function rewriteShell(template, route) {
  ...
  // v267 (founder-overdose strip): every bullet now leads with CAPABILITY
  // or CUSTOMER OUTCOME, not founder identity. Founder bio lives ONLY on
  // /about with the team page. This shell renders on every prerendered route,
  // so this is the highest-leverage place to reframe.
  const rootShell = `<div id="prerender-shell" style="padding:64px 24px;max-width:880px;margin:0 auto;font-family:'Inter Tight',system-ui,sans-serif;color:#0B1B2B;background:#FFFFFF;min-height:100vh"><div style="font-family:ui-monospace,Menlo,monospace;letter-spacing:0.18em;font-size:11px;font-weight:700;color:#C99A28;text-transform:uppercase;margin-bottom:14px">AI RECEPTIONIST · BUILT FOR SERVICE SMBs</div><h1 style="font-size:clamp(36px,6vw,64px);line-height:1.05;letter-spacing:-0.02em;font-weight:600;color:#042C53;margin:0 0 18px">${h1} <span style="font-family:'Playfair Display',Georgia,serif;font-style:italic;font-weight:500">${italic}</span></h1><p style="font-size:18px;line-height:1.55;color:#5C6B7F;margin:0 0 28px;max-width:640px">${sub}</p><div style="display:flex;flex-wrap:wrap;gap:12px"><a href="/apply" style="padding:14px 22px;border-radius:999px;background:#042C53;color:#fff;text-decoration:none;font-weight:700">Book a discovery call →</a><a href="/pricing" style="padding:14px 22px;border-radius:999px;background:transparent;color:#042C53;text-decoration:none;font-weight:700;border:1.5px solid #042C53">See the three lanes →</a></div><ul style="margin:32px 0 0;padding:0;list-style:none;display:grid;gap:10px;font-size:15px"><li><span style="color:#C99A28;font-weight:800">+ </span>Answers every call in under 2 rings — 24/7, no missed leak</li><li><span style="color:#C99A28;font-weight:800">+ </span>Qualifies the lead, books the appointment, sends the SMS confirmation</li><li><span style="color:#C99A28;font-weight:800">+ </span>Plugs into Cal.com / ServiceTitan / GHL / HubSpot out of the box</li><li><span style="color:#C99A28;font-weight:800">+ </span>HVAC, dental, salon, real estate, gym, hotels, and 19 more verticals</li><li><span style="color:#C99A28;font-weight:800">+ </span>Multi-LLM fallback (Anthropic → Groq → Gemini → OpenAI) so the line never goes silent</li><li><span style="color:#C99A28;font-weight:800">+ </span>Refund the build fee if we don't ship on time</li></ul></div>`;
