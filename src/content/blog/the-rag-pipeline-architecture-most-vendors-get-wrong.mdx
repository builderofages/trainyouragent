---
title: "The RAG Pipeline Architecture Most Vendors Get Wrong"
slug: "the-rag-pipeline-architecture-most-vendors-get-wrong"
date: "2026-04-23"
author: "Alexander Mills"
excerpt: "Chunk-and-embed is not RAG. Here is the 7-stage pipeline that actually grounds answers, with the eval harness that catches hallucinations before deploy."
category: "AI Infrastructure"
heroImage: "/api/og?title=The+RAG+Pipeline+Architecture+Most+Vendors+Get+Wrong&subtitle=TrainYourAgent"
tags: ["rag", "architecture", "pipelines", "embeddings", "eval"]
...
1. **Source.** Pull from system-of-record APIs. Not scrape. Not export-and-forget.
2. **Normalize.** Strip nav, footers, irrelevant DOM. Get to clean Markdown.
3. **Chunk.** Semantic chunking, not fixed-size. Use an LLM-as-chunker prompt that splits on topic boundaries.
4. **Enrich.** Attach metadata: source URL, last-modified, owner, doc-type, audience.
5. **Embed.** Voyage 3 large or text-embedding-3-large. Run on a schedule, not on-demand.
6. **Retrieve and re-rank.** Top-30 vector search, re-rank to top-5 with Cohere or a cross-encoder.
7. **Generate with citations.** The model is required to cite at least one source per claim. The UI shows the citations.
