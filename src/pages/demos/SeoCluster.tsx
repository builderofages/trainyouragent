// src/pages/demos/SeoCluster.tsx
// v42: live demo that hits /api/chat with mode="seo"
...
const PRESET = "ai voice agent for hvac";
...
export default function SeoCluster() {
  ...
  const runWith = async (seed: string) => {
    if (!seed) return;
    ...
    <div className="min-h-screen bg-white text-[#0B1B2B]" style={{ fontFamily: FONT }}>
      ...
      <main className="max-w-3xl mx-auto px-5 sm:px-8 py-10 sm:py-16">
        ...
        <p className="text-[16px] text-slate-600 leading-relaxed mb-8 max-w-2xl">
          Enter a seed keyword. The agent returns 20 related keywords grouped into 4 topical clusters with search-intent labels — the foundation of any programmatic SEO play.
        </p>

        <form onSubmit={submit} className="mb-6">
          <label htmlFor="seo-input" className="sr-only">Seed keyword</label>
          <input
            id="seo-input"
            type="text"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="e.g. ai voice agent for hvac"
          ...
          <div className="mt-3 flex flex-col sm:flex-row gap-2">
            ...
            >
              {status === "loading" ? "Clustering…" : "Generate clusters"}
            </button>
            <button
              type="button"
              onClick={tryPreset}
        ...
        {status === "loading" && !output && (
          <div role="status" aria-live="polite" className="text-[13px] text-slate-500">Clustering…</div>
        )}
        ...
        {status === "done" && (
          <ToolEmailGate
            ...
            reportName="your full 100-keyword cluster map (CSV) + a 6-month content plan"
            payload={{ tool: "seo", input: input.slice(0, 200) }}
          />
        )}

        <div className="mt-12 pt-8 border-t border-slate-200 text-[14px] text-slate-600">
          Want a 200-page programmatic SEO site built for you? <Link to="/contact" className="font-medium" style={{ color: BLUE }}>Book a 30-min build call</Link>.
        </div>
      </main>
    </div>
  );
